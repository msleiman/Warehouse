$(function(){
	...
	// When something on the search results page is clicked
	$( '.search-result-content' ).on( 'click', '.thumb-link, .product-action, .name-link, .save_for_later', function(){
		...
		// If the user has clicked to add the item to cart
		if ( $( this ).hasClass( 'quickviewbutton' ) )
		{
			...
			ga('main.send', 'event', 'Quick buy', 'Click', pidf + '_-_' + pnam );
		}

		// If the user has clicked to add the item to their wishlist
